...
void blur3x3()
{
  ...
  const float texOffset = discrete_weights.y / linear_weight * dFdyFine(texCoord.y);
...
void blur5x5()
{
  ...
  const float texOffset = (discrete_weights.x + 2 * discrete_weights.y) / linear_weight * dFdyFine(texCoord.y);
...
void blur7x7()
{
  ...
  const vec2 texOffsets = vec2(discrete_weights.y, 2 * discrete_weights.z + 3 * discrete_weights.w) / linear_weights * dFdyFine(texCoord.y);
